...
int main(void) {
  parse_config_file("config/prerun.craigb");
  cprdaq_init();

  _g_logfile_filename = "prerun_debug";    

  wait_for_all_links_to_come_up(_g_fibers_requested);
  global_reset();
  float wait_duration = 1;
  fprintf(_g_info, "waiting %.2f seconds for eeprom to be read...\n", 
	  wait_duration);

  usleep((unsigned int)(wait_duration * 1000000.0));
  //fprintf(_g_info, "press a key after all fiber links are up (steady green)\n");
  //getchar();
  wait_for_all_links_to_come_up(_g_fibers_requested);
  readout_all_pending_data();
  fprintf(_g_info, "sending configuration commands to FEE modules...\n");
  ...
  _g_should_soft_trigger = true;

  unsigned int threshold = _g_threshold_scan_low_limit;
  ...
  while (1) {
    ...
    if (1) {
      ...
      send_command_packet_to_all_enabled_channels(0xeeeee01a,threshold);
      threshold = (threshold + _g_threshold_scan_step_size);
      if (threshold > _g_threshold_scan_high_limit) {
        threshold = _g_threshold_scan_low_limit;
      }
      // the following is commented out for the fDIRC:
      //                                if (CAMAC_initialized) {
    ...
    //unsigned long long int dsf = disk_space_free(_g_location_of_raw_datafiles);
