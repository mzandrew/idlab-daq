...
// this worked for 83143+83215 M transfers without error with the firmware from 2010-04-11
...
//		float seconds = ((float) (done - start)) / CLOCKS_PER_SEC;
		if (seconds) {
			float kilobytes_per_second = 1.0e-3 * number_of_writes / seconds;
			printf("%d b in %4.1f s = %4.0f kbytes/s [w, then r; %d err in %4d M xfers; blocky]\n", number_of_writes, seconds, kilobytes_per_second, number_of_errors, millions_of_transfers);
		}
		number_of_writes = 0;
		number_of_reads = 0;
	}
...
void test_local_bus(void) {
	if (0) {
		test_local_bus__write_and_then_read__blocky();
	} else {
		while(1) {
//			test_local_bus__write_and_then_read__blocky();
...
//			test_local_bus__measure_write_and_then_read_data_rate__interleaved__single();
			test_local_bus__measure_write_and_then_read_data_rate__interleaved__blocky();
		}
	}
}
