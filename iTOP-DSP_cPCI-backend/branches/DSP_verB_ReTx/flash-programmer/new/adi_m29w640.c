...
const int data_bus_pins_zero_and_one_swapped = 0;
...
static ERROR_CODE ReadFlashCommand(unsigned long ulOffset, unsigned short *pusValue );
static ERROR_CODE ReadFlashData(unsigned long ulOffset, unsigned short *pusValue );
static ERROR_CODE WriteFlashCommand(unsigned long ulOffset, unsigned short usValue );
static ERROR_CODE WriteFlashData(unsigned long ulOffset, unsigned short usValue );
 			  		...
 			  		ADI_DEV_BUFFER		*pBuffer)
{
	...
	Result = ReadFlashData( *pulAbsoluteAddr, pusValue );
 			  		...
 			  		ADI_DEV_BUFFER		*pBuffer)
{
	...
	// unlock flash
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0xaa );
	WriteFlashCommand( ulFlashStartAddr + 0x0554, 0x55 );
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0xa0 );

	// program our actual value now
	Result = WriteFlashData( *pulAbsoluteAddr, *psValue );
...
ERROR_CODE ResetFlash(unsigned long ulAddr)
{
	...
	WriteFlashCommand( ulFlashStartAddr + 0x0554, 0xf0 );
...
ERROR_CODE EraseFlash(unsigned long ulAddr)
{
	...
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0xaa );
	WriteFlashCommand( ulFlashStartAddr + 0x0554, 0x55 );
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0x80 );
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0xaa );
	WriteFlashCommand( ulFlashStartAddr + 0x0554, 0x55 );
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0x10 );
...
ERROR_CODE EraseBlock( int nBlock, unsigned long ulAddr )
{
	...
	WriteFlashCommand( (ulFlashStartAddr + ulSectStart), 0x30 );
...
ERROR_CODE PollToggleBit(unsigned long ulAddr)
{
	...
	// read flash 1 time
	ReadFlashCommand( ulAddr, &usVal1 );

	while( ErrorCode == NO_ERR )
	{
		// read the value 2 times
		ReadFlashCommand( ulAddr, &usVal1 );
		ReadFlashCommand( ulAddr, &usVal2 );
...
ERROR_CODE GetCodes(int *pnManCode, int *pnDevCode, unsigned long ulAddr)
{
	...
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0xaa );
	WriteFlashCommand( ulFlashStartAddr + 0x0554, 0x55 );
	WriteFlashCommand( ulFlashStartAddr + 0x0AAA, 0x90 );

	ReadFlashCommand( ulFlashStartAddr + 0x0400, (unsigned short *)pnManCode );
	*pnManCode &= 0x00FF;
	 	
	// now we can read the codes
	ReadFlashCommand( ulFlashStartAddr + 0x0402, (unsigned short *)pnDevCode );
	*pnDevCode &= 0xFFFF;
...
ERROR_CODE ReadFlashCommand( unsigned long ulAddr, unsigned short *pusValue )
{
	...
	// read the value
	unsigned short int temp = (unsigned short)*pFlashAddr;
	if (data_bus_pins_zero_and_one_swapped)
	{
		*pusValue = (temp & 0xfffc) | ((temp & 0x0001)<<1) | ((temp & 0x0002)>>1);
//		*pusValue = (temp & 0xfffc) + ((temp & 0x0001)) + ((temp & 0x0002));
//		*pusValue = (temp & 0xfffc);
	} else {
		*pusValue = temp;
	}
...
ERROR_CODE ReadFlashData( unsigned long ulAddr, unsigned short *pusValue )
{
...
ERROR_CODE WriteFlashCommand( unsigned long ulAddr, unsigned short usValue )
{
	...
	if (data_bus_pins_zero_and_one_swapped)
	{
		*pFlashAddr = (usValue & 0xfffc) | ((usValue & 0x0001)<<1) | ((usValue & 0x0002)>>1);
//		*pFlashAddr = (usValue & 0xfffc) + ((usValue & 0x0001)) + ((usValue & 0x0002));
//		*pFlashAddr = (usValue & 0xfffc);
	} else {
		*pFlashAddr = usValue;
	}
...
ERROR_CODE WriteFlashData( unsigned long ulAddr, unsigned short usValue )
{
